...
func TestEncodingCommandsBSFullCoverage(t *testing.T) {
	...
	tests := []struct {
		...
		{"BSON.DECODE valid", "BSON.DECODE", [][]byte{{0x05, 0x00, 0x00, 0x00, 0x00}}},
		...
		{"CBOR.DECODE valid", "CBOR.DECODE", [][]byte{{0xbf, 0xff}}},
		...
		{"MSGPACK.DECODE valid", "MSGPACK.DECODE", [][]byte{{0x81, 0xa3, 0x6b, 0x65, 0x79, 0xa5, 0x76, 0x61, 0x6c, 0x75, 0x65}}},
		{"MSGPACK.DECODE invalid header", "MSGPACK.DECODE", [][]byte{{0x00}}},
		{"MSGPACK.DECODE incomplete", "MSGPACK.DECODE", [][]byte{{0xda, 0x00, 0x10}}},
...
func TestEncodingCommandsMSGPACKFullCoverage(t *testing.T) {
	...
	tests := []struct {
		...
		{"MSGPACK.ENCODE no args", "MSGPACK.ENCODE", nil},
		{"MSGPACK.DECODE simple", "MSGPACK.DECODE", [][]byte{{0x81, 0xa3, 0x6b, 0x65, 0x79, 0xa5, 0x76, 0x61, 0x6c, 0x75, 0x65}}},
		{"MSGPACK.DECODE empty", "MSGPACK.DECODE", [][]byte{{}}},
		{"MSGPACK.DECODE invalid", "MSGPACK.DECODE", [][]byte{{0x01, 0x02, 0x03}}},
		{"MSGPACK.DECODE no args", "MSGPACK.DECODE", nil},
	}
